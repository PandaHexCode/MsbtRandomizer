...
namespace CLMS
{
    public class MSBT : LMSBase, IYaml<MSBT>
    {
        ...
        public MSBT(Syroot.BinaryData.ByteOrder aByteOrder, Encoding aEncoding, bool createDefaultHeader = true) : base(aByteOrder, aEncoding, createDefaultHeader, FileType.MSBT) { }
            ...
            foreach (var rootChild in root.Children)
            {
                ...
                switch (key)
                {
                    case "Version":
                        ...
                        break;
                    case "IsBigEndian":
                        msbt.ByteOrder = bool.Parse(value) ? Syroot.BinaryData.ByteOrder.BigEndian : Syroot.BinaryData.ByteOrder.LittleEndian;
                        break;
                    case "UseIndices":
                    case "UseIndexes":
        ...
        private ATR1 ReadATR1(BinaryDataReader reader, long cSectionSize)
        {
            ...
            if (cSectionSize > (8 + (numOfAttributes * sizePerAttribute))) // if current section is longer than attributes, strings follow
            {
                ...
                foreach (byte[] cAttributeBytes in attributeBytesList)
                {
                    ...
                    if ((BitConverter.IsLittleEndian && reader.ByteOrder == Syroot.BinaryData.ByteOrder.BigEndian) ||
                        (!BitConverter.IsLittleEndian && reader.ByteOrder == Syroot.BinaryData.ByteOrder.LittleEndian))
                    {
        ...
        private void WriteATR1(BinaryDataWriter writer, Message[] messages)
        {
            ...
            if (UsesAttributeStrings)
            {
                ...
                for (int i = 0; i < messages.Length; i++)
                {
                    ...
                    // match system endianess with the BinaryDataReader if wrong
                    if ((BitConverter.IsLittleEndian && writer.ByteOrder == Syroot.BinaryData.ByteOrder.BigEndian) ||
                        (!BitConverter.IsLittleEndian && writer.ByteOrder == Syroot.BinaryData.ByteOrder.LittleEndian))
                    {
